...
export default class FakeCacheBuilder {
	...
	withFragmentString(cacheKey: string, fragment: string): FakeCacheBuilder {
		this.cache.fragmentCache.put(
			cacheKey,
			new FakeFragmentTemplate({ cacheKey, fragment }).create(),
		);
		return this;
	}
	...
	withQueryString(
		...
		this.cache.queryCache.put(
			cacheKey,
			new FakeQueryTemplate({ cacheKey, query, variables }).create(),
		);
		return this;
	}
